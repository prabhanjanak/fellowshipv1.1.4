...
async function main() {
  ...
  try {
    ...
    console.log(`Found ${specialities.length} specialities:`, specialities.map((s: any) => s.name));

    // Resolve specific speciality IDs or use fallback
    const glaucomaSpec = specialities.find((s: any) => s.name.toLowerCase().includes('glaucoma')) || specialities[0];
    const retinaSpec = specialities.find((s: any) => s.name.toLowerCase().includes('retina')) || specialities[1];
    const corneaSpec = specialities.find((s: any) => s.name.toLowerCase().includes('cornea')) || specialities[2];
    ...
    // 2. Fetch/Insert Doctor Users
    // Clean up old mock doctor records to prevent duplication or obsolete data
    console.log("Cleaning old mock doctor accounts...");
    await client.query("DELETE FROM interview_panel_members WHERE panel_id IN (SELECT id FROM interview_panels WHERE is_mock = true)");
    await client.query("DELETE FROM users WHERE role = 'doctor' AND email LIKE '%@fellowship.org'");
